...
export default function SearchBar({
  ...
  return (
    <div ref={containerRef} className="relative w-full max-w-2xl mx-auto">
      <div
        className={`flex items-center border px-5 py-4 gap-4 transition-all ${
          open
            ? "border-white bg-[#111111]"
            : "border-[#2a2a2a] bg-[#111111] hover:border-[#555555]"
        }`}
      >
        <svg className="w-5 h-5 text-[#555555] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        ...
        <input
          ...
          autoComplete="off"
          spellCheck={false}
          className="flex-1 text-lg text-white placeholder-[#444444] bg-transparent outline-none"
        />
        {query.length > 0 && (
          <button
        ...
        <div className="absolute top-full left-0 right-0 mt-px bg-[#111111] border border-[#2a2a2a] shadow-2xl overflow-hidden z-50">
          {results.map((result, i) => (
            <button
              ...
              className={`w-full text-left px-5 py-4 flex items-center justify-between gap-4 transition-colors border-t border-[#1a1a1a] first:border-t-0 ${
                i === highlighted ? "bg-[#1a1a1a]" : "hover:bg-[#161616]"
              }`}
            >
              <span className="text-base text-white font-medium">{result.club.name}</span>
              {result.kind === "english" ? (
                <span className={`text-sm tracking-[0.06em] uppercase shrink-0 ${DIVISION_COLORS[result.club.division]}`}>
                  {DIVISION_LABELS[result.club.division]}
                </span>
              ) : (
                <span className={`text-sm tracking-[0.06em] uppercase shrink-0 ${euColor(result.club)}`}>
                  {euLabel(result.club)}
                </span>
              )}
            </button>
