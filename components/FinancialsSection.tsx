...
// ─── Main component ───────────────────────────────────────────────────────────
export default function FinancialsSection({ club }: { club: ClubFinancials }) {
  const py = club.prior_year ?? null;
  const [fullView, setFullView] = useState<"table" | "chart">("table");

  // ── Chronological snapshots for trend dots (up to 4 years) ────────────────
  const snapList = [
  ...
  const hasMultiYear = !!(club.data2022 || club.data2023 || club.prior_year);
