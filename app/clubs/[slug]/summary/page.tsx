...
function calcHealthScore(club: ClubFinancials): {
  ...
  // 2. Profitability (operating margin)
  ...
  // 3. Debt position (net debt as multiple of revenue)
  ...
  // 4. Revenue trajectory (YoY growth)
  ...
  // 5. Pre-tax profit trend (direction vs prior year)
...
function buildInsights(club: ClubFinancials, plPeers: ClubFinancials[]): Insight[] {
  ...
  // Wage ratio ranking — ascending (1 = leanest)
  ...
  // Net debt ranking — descending (1 = most indebted)
  ...
  if (debtRank > 0 && debtRank <= 5 && club.net_debt != null) {
    const yoy = club.prior_year?.net_debt != null
      ? club.net_debt - club.prior_year.net_debt
      : null;
    const yoyStr = yoy != null
      ? ` (${yoy >= 0 ? "+" : ""}£${Math.abs(yoy).toFixed(0)}m YoY)`
      : "";
    candidates.push({
      title: "Debt burden",
  ...
  // Squad value ranking — descending (1 = highest)
  ...
  // Sort: red flags first, then green, then amber — take top 3
  return [
...
export default async function ClubSummaryPage({
  ...
  // Only Arsenal is supported at this stage
  if (slug !== "arsenal") notFound();
  ...
  // PL peers for ranking calculations
  ...
  // ── Badges (match existing HealthBadges logic) ────────────────────────────────
  ...
  return (
    <div className="px-6 lg:px-12 py-8">
      ...
      {/* Header: league/date/badges left, health score right */}
      <div className="flex flex-wrap items-start justify-between gap-6 mb-8">
        <div className="min-w-0">
          ...
          <p className="text-sm text-[#999999] mb-3">
            ...
            <span className="text-[#666666]">{fyDate}</span>
          </p>
          {(positives.length > 0 || issues.length > 0) && (
            <div className="flex flex-wrap gap-2">
              {positives.map(p => (
                <span
                  key={p}
                  className="inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-medium border border-[#4a9a6a] text-[#4a9a6a]"
                >
                  {p}
                </span>
              ))}
              {issues.map(i => (
                <span
                  key={i}
                  className="inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-medium border border-[#9a4a4a] text-[#9a4a4a]"
                >
                  {i}
                </span>
              ))}
        ...
        {/* Health score — most prominent element */}
        <div
        ...
        <DataCard title="Stadium & Market">
          ...
          <MetricRow
            ...
            value={
              market?.pop_m != null
                ? market.pop_m >= 1
                  ? `${market.pop_m.toFixed(1)}m`
                  : `${Math.round(market.pop_m * 1000)}k`
                : null
            }
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
          {insights.map((insight, i) => (
            <div
              ...
              style={{
                borderLeftColor: insightBorder(insight.sentiment),
                borderLeftWidth: 3,
              }}
            >
